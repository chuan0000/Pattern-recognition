...
for y in range(h):
    for x in range(w):
        ...
        g = int(image[1][x,y])
        r = int(image[2][x,y])
        if b>=40 and b<70 and g>=40 and g< 70 and r>=40 and r<70:
            c = (r,g,b)
        else:
            c=(0,0,0)
        im.putpixel((y,x),c)
im.save('duck180-2.tif')
...
tophat1 = cv2.morphologyEx(binduck180, cv2.MORPH_TOPHAT, kernel)
...
tophat2 = cv2.morphologyEx(binduck180, cv2.MORPH_TOPHAT, kernel)
...
tophat3 = cv2.morphologyEx(binduck180, cv2.MORPH_TOPHAT, kernel)
